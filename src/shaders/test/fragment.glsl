...
void main()
{
	...
	float strength = step(0.01, abs(distance(wavedUv, vec2(0.5)) - 0.25)) - 0.1;
	...
	// angle /= PI * 2.0;
	// angle += 0.5;
	// float strength = sin(angle * 100.0 - uTime * 10.0);

	// Pattern 45
	// float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
	// angle /= PI * 2.0;
	// angle += 0.5;
	// float sinusoid = sin(angle * 100.0 * cos(uTime));
